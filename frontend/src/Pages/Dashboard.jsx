...
function Dashboard() {
  ...
  const load = async () => {
      ...
      try {
        ...
        const myEvents = all.filter(ev => String(ev.createdBy) === String(user?._id) || String(ev.createdBy?._id) === String(user?._id));
        ...
        const withCounts = myEvents.map(ev => {
          ...
          if (!derivedCreatedAt && ev._id && typeof ev._id === 'string' && ev._id.length >= 8) {
            try {
          ...
          return { ...ev, attendees: match ? match.count : 0, _createdAt: derivedCreatedAt };
        });
  ...
  setFilteredEvents(withCounts); // initial
        ...
        const upcoming = withCounts.filter(ev => new Date(ev.date).getTime() >= now).length;
        const attendees = withCounts.reduce((s, e) => s + (Number(e.attendees) || 0), 0);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setEvents(prev => {
        ...
        const upcoming = updated.filter(ev => new Date(ev.date).getTime() >= now).length;
        const attendees = updated.reduce((s, e) => s + (Number(e.attendees) || 0), 0);
        setStats({ total, upcoming, attendees });
        return updated;
      });
  ...
  useEffect(() => {
    ...
    const getCreated = (e) => new Date(e._createdAt || e.createdAt || (e._id && typeof e._id === 'string' && e._id.length >= 8 ? parseInt(e._id.substring(0,8),16)*1000 : e.date || 0));
    const sorter = {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', fontFamily: 'Silevena' }}>
      <Header />
      <main className="layout-container" style={{ flex: 1, padding: '2.5rem 0' }}>
        <header style={{ display: 'flex', flexDirection:'column', gap: '1.25rem' }}>
          <div>
            <h1 className="ed-title" style={{ margin: 0 }}>Admin Dashboard</h1>
            <p className="text-muted" style={{ margin: 0 }}>Welcome {user?.username} — manage and analyze your events.</p>
          </div>
          {/* Unified styled search / filter bar */}
          <div className={`dash-search fx-bar`} style={{ padding:'14px 20px' }}>
            <div className="eventsPage-searchRow fx-inner" style={{ justifyContent:'flex-start' }}>
              {/* Search */}
              <div className="eventsPage-searchBox fx-item" style={{ flex:'2 1 260px' }}>
              ...
              {/* Category */}
              <div className="fx-selectWrap fx-item" style={{ flex:'1 1 170px' }}>
                <select
              ...
              <div className="fx-feeToggle fx-item" role="group" aria-label="Filter by attendees" data-count="4" style={{ minWidth:'230px' }}>
                {[
                  { label:'ALL', val:''},
                  { label:'10+', val:'10'},
                  { label:'50+', val:'50'},
                ...
                <span
                  ...
                  style={{ '--fx-index': minAttendees === '10' ? 1 : minAttendees === '50' ? 2 : minAttendees === '100' ? 3 : 0, width:'calc((100% - 16px - 16px)/4)' }}
                />
              </div>
              {/* Sort controls */}
              <div className="fx-sortControls fx-item" style={{ display:'flex', gap:'8px' }}>
                <select
                ...
                <button
                  ...
                  title={`Toggle sort direction (${sortDir === 'asc' ? 'Ascending' : 'Descending'})`}
                  aria-label="Toggle sort direction"
                >{sortDir === 'asc' ? '↑' : '↓'}</button>
              </div>
              {(searchQuery || categoryFilter || minAttendees !== '' || sortOption !== 'new' || sortDir !== 'desc') && (
                <button className="btn btn-danger fx-item" style={{ marginLeft:'8px' }} onClick={() => { setSearchQuery(''); setCategoryFilter(''); setMinAttendees(''); setSortOption('new'); setSortDir('desc'); }}>Reset</button>
              )}
            </div>
          </div>
        </header>
        ...
        <section style={{ marginTop: '1.25rem' }}>
          ...
          {error && <div className="surface p-md" style={{ color: 'var(--accent-color)' }}>{error}</div>}

          <div className="grid grid-cols-3" style={{ marginTop: '1rem' }}>
            <div className="surface p-md">
              <div className="text-muted" style={{ fontSize: '.75rem', textTransform: 'uppercase', fontWeight: 700 }}>Events</div>
              <div style={{ fontSize: '1.6rem', fontWeight: 700 }}>{stats.total}</div>
            </div>
            <div className="surface p-md">
              <div className="text-muted" style={{ fontSize: '.75rem', textTransform: 'uppercase', fontWeight: 700 }}>Upcoming</div>
              <div style={{ fontSize: '1.6rem', fontWeight: 700 }}>{stats.upcoming}</div>
            </div>
            <div className="surface p-md">
              <div className="text-muted" style={{ fontSize: '.75rem', textTransform: 'uppercase', fontWeight: 700 }}>Attendees</div>
              <div style={{ fontSize: '1.6rem', fontWeight: 700 }}>{stats.attendees}</div>
            </div>
          </div>
          ...
          <section style={{ marginTop: '1.5rem' }}>
            ...
            {filteredEvents.length === 0 && !loading ? (
              <div className="surface p-md">You haven't created any events yet.</div>
            ) : (
              <div style={{ overflowX: 'auto' }}>
                <table className="dashboard-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    ...
                    {!loading && filteredEvents.slice(currentPage * perPage, (currentPage + 1) * perPage).map(ev => (
                      <tr key={ev._id} style={{ borderBottom: '1px solid var(--border-light)' }}>
                        <td style={{ padding: '12px' }}>{ev.title}</td>
                        <td style={{ padding: '12px' }}>{new Date(ev.date).toLocaleString()}</td>
                        <td style={{ padding: '12px' }}>{ev.location}</td>
                        <td style={{ padding: '12px' }}>{ev.attendees ?? 0}</td>
                        <td style={{ padding: '12px' }}>
                          <div className="action-buttons">
                            <a href={`/events/${ev._id}/edit`} className="action-btn btn-outline" title="Edit">
                              <FaEdit style={{ marginRight: 8 }} />
                              <span>edit</span>
                            </a>
                            <button className="action-btn btn-danger" onClick={() => handleDelete(ev._id)} title="Delete">
                    ...
                    <div className="per-page">
                      <label>Show
                        <select value={perPage} onChange={e => { setPerPage(Number(e.target.value)); setCurrentPage(0); }}>
                          {[6, 10, 15, 20].map(v => <option key={v} value={v}>{v}</option>)}
                        </select>
                      </label>
                    </div>
                    <button className="btn btn-outline" onClick={() => setCurrentPage(p => Math.max(0, p - 1))} disabled={currentPage === 0}>
                      <FaChevronLeft /> Prev
                    </button>
                    <div className="page-numbers">
                      {Array.from({ length: Math.ceil(filteredEvents.length / perPage) }).map((_, i) => (
                        <button key={i} className={"page-btn" + (i === currentPage ? ' active' : '')} onClick={() => setCurrentPage(i)}>{i + 1}</button>
                      ))}
                    </div>
                    <div className="page-info">Page {currentPage + 1} / {Math.ceil(filteredEvents.length / perPage)}</div>
                    <button className="btn btn-outline" onClick={() => setCurrentPage(p => Math.min(Math.ceil(filteredEvents.length / perPage) - 1, p + 1))} disabled={currentPage >= Math.ceil(filteredEvents.length / perPage) - 1}>
                      Next <FaChevronRight />
                    </button>
                  </div>
                )}
        ...
        <div className="modal-overlay" role="dialog" aria-modal="true">
          <div className="modal">
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 12 }}>
              <button className="btn btn-outline" onClick={cancelDelete} disabled={deleteLoading}>Cancel</button>
              <button className="btn btn-danger" onClick={confirmDelete} disabled={deleteLoading}>{deleteLoading ? 'Deleting...' : 'Delete'}</button>
            </div>
          </div>
        </div>
      )}
